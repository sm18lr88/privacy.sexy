...
export function itEachAbsentStringValue(
  runner: (absentValue: never) => void,
  options: AbsentTestCaseOptions = DefaultAbsentTestCaseOptions,
): void {
  itEachAbsentTestCase(getAbsentStringTestCases(options), (value) => {
    Reflect.apply(runner, undefined, [value]);
  });
}
...
export function itEachAbsentCollectionValue<T>(
  ...
  itEachAbsentTestCase(getAbsentCollectionTestCases<T>(options), (value) => {
    Reflect.apply(runner, undefined, [value]);
  });
}
...
export function itEachAbsentObjectValue(
  ...
  itEachAbsentTestCase(getAbsentObjectTestCases(options), (value) => {
    Reflect.apply(runner, undefined, [value]);
  });
}
